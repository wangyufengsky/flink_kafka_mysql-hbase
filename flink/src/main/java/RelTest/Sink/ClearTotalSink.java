...
public class ClearTotalSink extends RichSinkFunction<List<clearBean>> implements Serializable {
    ...
    private static List<ClearTotalInsertBean> clearTotalInsertBeans =new ArrayList<>();
    ...
    private static void createTableMap(){
        tableMap.put("0","clear_total_0");
        tableMap.put("1","clear_total_1");
        tableMap.put("2","clear_total_2");
        tableMap.put("3","clear_total_3");
        tableMap.put("4","clear_total_4");
        tableMap.put("5","clear_total_5");
        tableMap.put("6","clear_total_6");
        tableMap.put("7","clear_total_7");
        tableMap.put("8","clear_total_8");
        tableMap.put("9","clear_total_9");
    }
    ...
    private static void createBeans(){
        for(int i=0;i<28;i++){
            for(int j=0;j<10;j++){
                clearTotalInsertBeans.add(new ClearTotalInsertBean(String.valueOf(i),String.valueOf(j)));
            }
        }
    }
    ...
    private void doDeal(List<clearBean> list){
        ...
        for(clearBean bean:list){
            for(ClearTotalInsertBean dataBaseTableBean: clearTotalInsertBeans){
                if(dataBaseTableBean.isBelong(bean)){
                    dataBaseTableBean.addBean(bean);
                }
            }
        }
        Map<String,List<ClearTotalInsertBean>> map= clearTotalInsertBeans.stream().parallel().collect(Collectors.groupingBy(ClearTotalInsertBean::getDataBase));
        for(Map.Entry<String,List<ClearTotalInsertBean>> entry:map.entrySet()){
    ...
    private void dataBaseInsert(List<ClearTotalInsertBean> list,String data){
        int i=0;
        for(ClearTotalInsertBean s:list){
        ...
        Map<String,List<ClearTotalInsertBean>> map=list.stream().parallel().collect(Collectors.groupingBy(ClearTotalInsertBean::getTableName));
        try (Statement statement = connection.createStatement()) {
            for(Map.Entry<String,List<ClearTotalInsertBean>> entry:map.entrySet()){
                for (ClearTotalInsertBean databean:entry.getValue()){
                    List<clearBean> beans=databean.getClearBeans();
                    if(beans.size()==0){
                        return;
                    }
                    StringBuffer sql=new StringBuffer("INSERT INTO ").append(entry.getKey()).append(" (event_id,cst_no,create_date,create_time,clear_total) VALUES ");
                    int times=0;
