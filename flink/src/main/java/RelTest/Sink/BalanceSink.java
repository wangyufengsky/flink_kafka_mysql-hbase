...
public class BalanceSink extends RichSinkFunction<List<clearBean>> implements Serializable {
    ...
    private static List<BalanceInsertBean> balanceInsertBeans =new ArrayList<>();
    ...
    private static void createTableMap(){
        tableMap.put("0","balance_0");
        tableMap.put("1","balance_1");
        tableMap.put("2","balance_2");
        tableMap.put("3","balance_3");
        tableMap.put("4","balance_4");
        tableMap.put("5","balance_5");
        tableMap.put("6","balance_6");
        tableMap.put("7","balance_7");
        tableMap.put("8","balance_8");
        tableMap.put("9","balance_9");
    }
    ...
    private static void createBeans(){
        for(int i=0;i<28;i++){
            for(int j=0;j<10;j++){
                balanceInsertBeans.add(new BalanceInsertBean(String.valueOf(i),String.valueOf(j)));
            }
        }
    }
    ...
    private void doDeal(List<clearBean> list){
        ...
        for(clearBean bean:list){
            for(BalanceInsertBean balanceInsertBean:balanceInsertBeans){
                if(balanceInsertBean.isBelong(bean)){
                    balanceInsertBean.addBean(bean);
                }
            }
        }
        Map<String,List<BalanceInsertBean>> map=balanceInsertBeans.stream().parallel().collect(Collectors.groupingBy(BalanceInsertBean::getDataBase));
        for(Map.Entry<String,List<BalanceInsertBean>> entry:map.entrySet()){
            dataBaseInsert(entry.getValue(),entry.getKey());
        }
        for(BalanceInsertBean dataBaseTableBean: balanceInsertBeans){
    ...
    private void dataBaseInsert(List<BalanceInsertBean> list,String data){
        int i=0;
        for(BalanceInsertBean s:list){
        ...
        Map<String,List<BalanceInsertBean>> map=list.stream().parallel().collect(Collectors.groupingBy(BalanceInsertBean::getTableName));
        try (Statement statement = connection.createStatement()) {
            for(Map.Entry<String,List<BalanceInsertBean>> entry:map.entrySet()){
                for (BalanceInsertBean databean:entry.getValue()){
                    List<clearBean> beans=databean.getClearBeans();
                    if(beans.size()==0){
                        return;
                    }
                    StringBuffer sql=new StringBuffer("INSERT INTO ").append(entry.getKey()).append(" (event_id,cst_no,busi_code,create_date,create_time,balance_amount) VALUES ");
                    int times=0;
