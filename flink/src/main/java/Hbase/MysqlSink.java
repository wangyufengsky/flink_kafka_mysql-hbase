...
public class MysqlSink extends RichSinkFunction<List<clearBean>> implements Serializable {
    ...
    private void doDeal(List<clearBean> list){
        ...
        list.parallelStream().forEach(a->dataBaseTableBeans.parallelStream().forEach(b->{
            if(b.isBelong(a)){
                b.addBean(a);
            }
        }));
        dataBaseTableBeans.stream().parallel().collect(Collectors.groupingBy(DataBaseTableBean::getDataBase)).forEach((key,value)-> dataBaseInsert(value,key));
        for(DataBaseTableBean dataBaseTableBean:dataBaseTableBeans){
    ...
    private void dataBaseInsert(List<DataBaseTableBean> list,String data){
        Connection connection=connectionMap.get(data);
        Map<String,List<DataBaseTableBean>> map=list.stream().parallel().collect(Collectors.groupingBy(DataBaseTableBean::getTableName));
        try (Statement statement = connection.createStatement()) {
            for(Map.Entry<String,List<DataBaseTableBean>> entry:map.entrySet()){
                for (DataBaseTableBean databean:entry.getValue()){
